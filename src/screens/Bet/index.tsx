  ...
  const handleAddToCart = useCallback(() => {
    ...
    if (numbers.length === limit) {
      ...
      Toast.show({
        type: "info",
        position: "top",
        text1: `Faltando ${limit - numbers.length} ${limit - numbers.length === 1 ? "número" : "números"
          } na sua jogada.`,
        text2: `É preciso escolher ${limit} números para finalizar esta jogada.`,
      });
    }
  }, [choosedNumbers, dispatch, color, gameId, limit, selectedGame, price]);
          ...
          <S.Filters horizontal showsHorizontalScrollIndicator={false}>
            {games.map((game) => (
              <GameButton
                ...
                color={game.color}
                gameType={game.type}
                isFocused={game.type === selectedGame}
              />
            ))}
          </S.Filters>
              ...
              <>
                ...
                <S.Description>
                  {description.length === 0
                    ...
                    ? "Chosse a game to fill your bet."
                    : description}
                </S.Description>
              </>
            )}
          ...
          <S.Numbers
            ...
            renderItem={({ item }) => (
              <BetNumber
                ...
                isFocused={choosedNumbers.includes(item + 1)}
              />
            )}
            numColumns={5}
          />
